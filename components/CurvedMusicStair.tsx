...
export default function CurvedMusicStair() {
  ...
  const parts = useMemo(() => {
    ...
    const flight = new THREE.CubicBezierCurve3(
      new THREE.Vector3(9.5,0,-2.8), new THREE.Vector3(13.2,0,-7.2),
      new THREE.Vector3(5.7,0,-5.2), new THREE.Vector3(5.7,0,-11.5),
    );
    const at = (t: number) => { const p=flight.getPoint(t); p.y=-1.75+t*6.72; return p; };
    const end = at(1);
    ...
    const gallery = new THREE.CatmullRomCurve3([
      end, landingLead, new THREE.Vector3(8.6,4.97,-17.5),
    ...
    const steps=Array.from({length:stepCount},(_,i)=>{
      ...
      const corners=[[a,-1],[b,-1],[b,1],[a,1]].map(([t,sign])=>
        at(t).addScaledVector(flight.getTangent(t).cross(up).normalize(),sign*1.1));
    ...
    return {deck,steps,rails,posts,columns:[.2,.43,.64,.83,1].map(t=>gallery.getPoint(t))};
  },[aspect]);
